...
namespace app {
	...
	public:
		...
		string to_string() const override {
			return "[" + std::to_string(_age) + ", " + _name + "]";
		}
	};
} // namespace app

template <> struct std::hash<app::Person> {
	size_t operator()(const app::Person& p) const {
		return std::hash<string>()(p._name) ^ std::hash<int>()(p._age);
	}
};
