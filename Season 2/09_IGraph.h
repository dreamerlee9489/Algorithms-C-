...
namespace app
{
	...
	struct EdgeInfo
	{
		friend std::ostream& operator<<(std::ostream& out, const EdgeInfo& edge) { return out << "EdgeInfo <" << *edge._from << ", " << *edge._to << ", " << edge._weight << ">"; }
		friend bool operator==(const EdgeInfo& lhs, const EdgeInfo& rhs) { return *lhs._from == *rhs._from && *lhs._to == *rhs._to && lhs._weight == rhs._weight; }
