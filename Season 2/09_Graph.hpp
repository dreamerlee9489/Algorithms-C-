...
namespace app {
    ...
    struct Edge {
        friend ostream &operator<<(ostream &out, const Edge &edge) {
            return out << "Edge [" << *edge._from << ", " << *edge._to << ", " << edge._weight << "]";
        }
    ...
    class Graph : public IGraph<V> {
        ...
        void relaxation(Edge<V> *edge, PathInfo<V> *fromPath, unordered_map<Vertex<V>, PathInfo<V>> *pathMap) {
            ...
            typename unordered_map<Vertex<V>, PathInfo<V>>::iterator iter = pathMap->find(*edge->_to);
    ...
    public:
        ...
        void add_vertex(V *v) override { _vertexMap.emplace(make_pair(*v, Vertex<V>(v))); }

        void add_edge(V *from, V *to, double weight = 0) override {
            typename unordered_map<V, Vertex<V>>::iterator iter = _vertexMap.find(*from);
        ...
        void remove_vertex(V *v) override {
            ...
            _vertexMap.erase(*v);
            if (vertex != nullptr) {
                for (typename unordered_set<Edge<V>>::iterator iter = vertex->_outDegree.begin();
                     iter != vertex->_outDegree.end(); ++iter) {
                    iter->_to->_inDegree.erase(*iter);
                    _edgeSet.erase(*iter);
                    vertex->_outDegree.erase(*iter);
                }
                for (typename unordered_set<Edge<V>>::iterator iter = vertex->_inDegree.begin();
        ...
        void bfs(V *begin, typename IGraph<V>::TraverseFunc func = nullptr) override {
            typename unordered_map<V, Vertex<V>>::iterator iter = _vertexMap.find(*begin);
        ...
        vector<V> *topo_sort() override {
            ...
            for (typename unordered_map<V, Vertex<V>>::iterator iter = _vertexMap.begin();
            ...
            if (beginVertex != nullptr) {
                ...
                while (!pathMap->empty()) {
                    ...
                    for (Edge<V> edge: minVertex._outDegree) {
                        if (selectedPathMap->find(*edge._to->_value) == selectedPathMap->end())
...
struct hash<app::Edge<V>> {
    size_t operator()(const app::Edge<V> &edge) const {
        return hash<app::Vertex<V>>()(*edge._from) ^ hash<app::Vertex<V>>()(*edge._to);
    }
};
