...
namespace app {
    ...
    public:
        ...
        void remove_vertex(V *v) override {
            ...
            vertex_map.erase(*v);
            if (vertex != nullptr) {
                for (typename unordered_set<Edge<V>>::iterator iter = vertex->out_degree.begin();
                     iter != vertex->out_degree.end(); ++iter) {
                    iter->_to->in_degree.erase(*iter);
                    edge_set.erase(*iter);
                    vertex->out_degree.erase(*iter);
                }
                for (typename unordered_set<Edge<V>>::iterator iter = vertex->in_degree.begin();
                     iter != vertex->in_degree.end(); ++iter) {
