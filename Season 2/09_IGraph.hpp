...
namespace app {
	...
	template <typename V> struct PathInfo {
		double _weight;
		std::vector<EdgeInfo<V>> _edgeInfos;

		friend std::ostream& operator<<(std::ostream& out, const PathInfo& path) {
	 ...
	 * @date 2022-05-09
	 */
	template <typename V> class IGraph {
	protected:
...
template <typename V> struct std::hash<app::EdgeInfo<V>> {
	size_t operator()(const app::EdgeInfo<V>& edge) const {
		return hash<V>()(*edge._from) ^ hash<V>()(*edge._to) ^
			hash<double>()(edge._weight);
	}
};
