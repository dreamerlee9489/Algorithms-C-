...
namespace app {
	...
	template <typename V> class Trie {
		template <typename _V> struct Node {
			bool _word = false;
	...
	template <typename V> shared_ptr<V> Trie<V>::remove(string key) {
		shared_ptr<Node<V>> node = get_node(key);
	...
	template <typename V>
	shared_ptr<typename Trie<V>::template Node<V>>
		Trie<V>::get_node(string key) const {
		key_null_check(key);
		shared_ptr<Node<V>> node = _root;
		for (size_t i = 0; i < key.size(); ++i) {
