...
namespace app {
	 ...
	 * @tparam V
	 */
	template <typename K, typename V> class LinkedHashMap : public HashMap<K, V> {
		using NODE = typename HashMap<K, V>::template Node<K, V>;

		template <typename _K, typename _V> struct LinkedNode : public NODE {
		...
		LinkedNode<K, V>* create_node(shared_ptr<K> key, shared_ptr<V> value,
	...
	public:
		...
		void
			traverse(typename HashMap<K, V>::TraverseFunc func = nullptr) const override;
	...
	inline typename LinkedHashMap<K, V>::template LinkedNode<_K, _V>&
		LinkedHashMap<K, V>::LinkedNode<_K, _V>::operator=(
			const LinkedNode<_K, _V>& node) {
		this->_key = node._key;
	...
	inline typename LinkedHashMap<K, V>::template LinkedNode<_K, _V>&
		LinkedHashMap<K, V>::LinkedNode<_K, _V>::operator=(
			LinkedNode<_K, _V>&& node) noexcept {
		this->_key = nullptr;
		this->_value = nullptr;
		this = &node;
		return *this;
	...
	template <typename K, typename V>
	inline typename LinkedHashMap<K, V>::template LinkedNode<K, V>*
		LinkedHashMap<K, V>::create_node(shared_ptr<K> key, shared_ptr<V> value,
			NODE* parent) {
	...
	template <typename K, typename V>
	inline void LinkedHashMap<K, V>::after_remove_derived(NODE* willnode,
		NODE* rmvnode) {
	...
	template <typename K, typename V>
	inline void
		LinkedHashMap<K, V>::traverse(typename HashMap<K, V>::TraverseFunc func) const {
		LinkedNode<K, V>* node = _head;
		while (node != nullptr) {
			if (func != nullptr)
